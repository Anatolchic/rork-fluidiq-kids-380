...
import { loadDirectChatsForStudent, DirectChatRow } from '../../lib/direct-chats';
...
export default function StudentMessagesScreen() {
  ...
  useEffect(() => {
    ...
    // realtime: новое сообщение в любой чат → перезагрузка списка
    const channel = supabase
      .channel(`student_direct_chats:${session.user.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'direct_chats', filter: `student_id=eq.${session.user.id}` },
        () => { load(); },
      )
      .subscribe();
  ...
  async function load() {
    if (!session) return;
    const { data, error } = await loadDirectChatsForStudent(session.user.id);
    if (error || !data) {
      setItems([]);
      setLoading(false);
      setRefreshing(false);
    ...
    const { data: tutors } = await supabase
      ...
      .in('user_id', tutorIds);
    const tMap: Record<string, { name: string; photo_url: string | null }> = {};
    (tutors || []).forEach((t: any) => {
      tMap[t.user_id] = { name: t.name, photo_url: t.photo_url };
    });
    const enriched: ChatItem[] = rows.map((r) => ({
      ...r,
      partner_name: tMap[r.tutor_id]?.name || 'Репетитор',
      partner_photo: tMap[r.tutor_id]?.photo_url || null,
    }));
    setItems(enriched);
    setLoading(false);
    setRefreshing(false);
  ...
  const renderItem = useCallback(
    ({ item }: { item: ChatItem }) => {
      const unread = item.student_unread > 0;
      return (
        <Pressable
              ...
              {unread && (
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>
                    {item.student_unread > 99 ? '99+' : item.student_unread}
                  </Text>
                </View>
              )}
            </View>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>
              Откройте профиль репетитора и нажмите «Написать»
            </Text>
          </View>
        }
      />
